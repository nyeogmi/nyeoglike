...
def residence() -> Carve:
    ...
    living_room = carve.carve(
        V2.new(0, 0).sized(V2.new(living_room_width, living_room_height)),
        RoomType.LivingRoom,
    )
    ...
    def build_kitchen(snake: Snake):
        ...
        carve.freeze(
            snake.tunnel(
                V2.new(kitchen_width, kitchen_height),
                RoomType.Kitchen,
                LinkType.Complete,
                min_contact=kitchen_width,
            )
        )
    ...
    def build_antechamber_once(snake: Snake):
        ...
        if choice == 0:
            ...
            carve.freeze(
                snake.tunnel(
                    V2.new(3, 3), RoomType.EntryZone, LinkType.Door, min_contact=3
                )
            )
        else:
            # yard-like
            carve.freeze(
                snake.tunnel(
                    V2.new(living_room_height, 3),
                    RoomType.Antechamber,
                    LinkType.Door,
                    min_contact=living_room_height,
                )
            )
            if random.choice([False, True]):
                snake.turn_right()
                carve.freeze(
                    snake.tunnel(V2.new(3, 3), RoomType.Antechamber, LinkType.Door)
                )
    ...
    def build_hall_room(snake: Snake, master: bool, depth: int) -> bool:
        for sz_x in range(cell_sz_x, 2, -1):
            room_type = (
                RoomType.Bedroom
                if master
                else RoomType.Bathroom
                if len(carve.ident_rooms(RoomType.Bathroom)) == 0
                else RoomType.Bedroom
            )
            size = V2.new(
                sz_x + (random.randint(2, 4) if master else 0),
                depth + (random.randint(0, 1) if master else 0),
            )
    ...
    def build_closet(snake: Snake) -> bool:
        for sz_x in range(5, 1, -1):
            ...
            with snake.veto_point() as vetoed:
                ...
                r = snake.tunnel(
                    size, room_type, LinkType.Door, min_contact=sz_x, rule=Rule.Dense
                )
