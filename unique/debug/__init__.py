...
def dump_world(w: World):
    ...
    def if_(c, x): return [x] if c else []
    ...
    for npch in sorted(w.npcs.all(), key=lambda n: w.npcs.get(n).name):
        ...
        with obj.prefix("household"):
            ...
            for i, fellow in enumerate(sorted(w.households.members(household), key=lambda n: w.npcs.get(n).name)):
                obj.add(i, w.npcs.get(fellow).name)

        with obj.prefix("shifts"):
            for i, shift in enumerate(w.enterprises.get_shifts_worked_by(npch)):
                shift_s = "{} ({})".format(w.enterprises.get_name(shift.enterprise), w.enterprises.get_shift(shift))
                obj.add(i, shift_s)
    ...
    for enterprise in sorted(w.enterprises.all(), key=lambda e: w.enterprises.get_name(e)):
